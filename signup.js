...
window.onload = function(){
    ...
    const passwordv = () => {
        ...
        var passwordRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9]).{5,}$/;
        if(passwordVal === "") {
            setErrorMsg(password, 'password cannot be blank');
        }else if (passwordVal.length >= 20 ) {
            setErrorMsg(password, 'password max 20 char');
        } else if (!passwordRegex.test(passwordVal)) {
            setErrorMsg(password, 'Password contain at least 5 character., at least 1 lower & upper case letter, 1 number, and 1 special character.');
        }else{
    ...
    const sendData = (usernameVal, cpasswordVal,emailVal,phoneVal,passwordVal,gen, sRate, count) => {
        if (sRate === count){
    ...
    const successMsg = (usernameVal,cpasswordVal,emailVal,phoneVal,passwordVal,gen) => {
        ...
        for(var i = 0; i < formCon.length; i++){
            if(formCon[i].className === "form-control success") {
                var sRate = 0 + i;
                console.log(sRate);
                sendData(usernameVal,cpasswordVal,emailVal,phoneVal,passwordVal,gen, sRate, count);
            }else{
                return false;
            }
        }
    ...
    function validate() {
        ...
        successMsg(usernameVal,cpasswordVal,emailVal,phoneVal,passwordVal,gen);
